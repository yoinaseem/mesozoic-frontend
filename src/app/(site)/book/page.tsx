...
function BookingPageInner() {
  ...
  // cart fields doesn't re-fire the network call. Dialog only opens for
  // window changes that haven't been answered for yet.
  ...
  useEffect(() => {
    ...
    if (!tripWindow || roomAlreadyExists) return;
    const key = `${tripWindow.checkIn}|${tripWindow.checkOut}`;
    if (lastLookupRef.current === key) return;
    lastLookupRef.current = key;

    let cancelled = false;
    findReservationsOverlapping(tripWindow.checkIn, tripWindow.checkOut)
      .then((matches) => {
        if (cancelled) return;
        if (matches.length === 0) {
    ...
    <div className="bg-base min-h-screen pt-24 pb-12">
      <div className="mx-auto w-full max-w-7xl px-6">
        <header className="mb-8">
          ...
          {roomAlreadyExists && primaryRoom ? (
            <p className="text-primary mt-3 text-sm font-semibold">
              Adding to your existing trip at {primaryRoom.hotel.name},{" "}
              {primaryRoom.checkIn} – {primaryRoom.checkOut}.
            </p>
        ...
        {showSelector ? (
          <UpcomingTripSelector
            ...
            onSelect={(reservation, anchor) => {
              ...
              // Move past the room step automatically — the room is
              // already booked, the next thing they want is a ticket.
              setActiveStep("ferry");
            }}
          />
        ) : null}
      ...
      <AttachReservationDialog
        ...
        proposedCheckIn={tripWindow?.checkIn ?? ""}
        proposedCheckOut={tripWindow?.checkOut ?? ""}
